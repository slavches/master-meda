...
import { loadFull } from "tsparticles"; // Полная версия для всех функций
...
const HeroSection = () => {
  ...
  useEffect(() => {
    initParticlesEngine(async (engine) => {
      await loadFull(engine); // Загружаем всё необходимое
    }).then(() => {
      setInit(true);
    });
  }, []);
  ...
  const options = useMemo(
    () => ({
      ...
      particles: {
        color: {
          value: ["#FFA000", "#FFCA28", "#DAA520", "#FFD700"],
        },
        links: {
          enable: false,
        },
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center text-center overflow-hidden bg-gradient-to-br from-amber-50 to-yellow-100">
      ...
      {/* Анимация капель мёда */}
      {init && <Particles id="honey-canvas" options={options} className="absolute inset-0 z-0" />}
      ...
      <div className="relative z-10 container mx-auto px-6">
        <h1 className="text-5xl md:text-6xl font-bold mb-6 text-amber-900">Мастер Мёда</h1>
        ...
        <button
          ...
          className="inline-block bg-amber-600 hover:bg-amber-700 text-white font-bold py-4 px-10 rounded-lg text-xl transition"
        >
